...
/*
 ./convert2TFileMulti inputFilePrefix StartFileID NumberOfFiles OutputFileName
 */
...
int main(int argc, char** argv) {
	...
	//----------------------------------------------
	TFile* otf = new TFile(argv[4], "RECREATE");
	/*
	TTree* str = new TTree("Setting", "Setting");
	str->Branch("NChannels", &nChannels, "NChannels/I");
	str->Branch("NSamples", &nSamples, "NSamples/I");
	str->Branch("AbsTime", &absInitX, "AbsTime/D");
	str->Branch("RelTime", &RelInitX, "RelTime/D");
	str->Branch("TimePitch", &XInc, "TimePitch/D");
	*/
	//-----------------Vector-----------------------
	std::vector< double > tVec;
	std::vector< double > vVec;
	TTree* otr = new TTree("WFM", "WFM");
	otr->Branch("Time", &tVec);
	otr->Branch("WFM", &vVec);
	//----------------------------------------------
	...
	WAVE_DESC wd;
	for (int fileNo=StartFileNumber; fileNo<StartFileNumber+nFiles; fileNo++) {
		std::cout << fileNo << "\r" << std::flush;
		snprintf( filename, 15, "%s%05d.trc", argv[1],fileNo );
		...
		tVec.resize(wf_length);
		vVec.resize(wf_length);
		...
		for (seg=0;seg < wd.subarray_count;seg++) {
			...
			if (wd.comm_type==0) {
				...
				for (i=0;i<wf_length;i++) {
					...
					//fputs(fileBuf,fp);
					tVec.at(i) = i*wd.horiz_interval+trigtime;
					vVec.at(i) = tmp*wd.vertical_gain-wd.vertical_offset;
				};
